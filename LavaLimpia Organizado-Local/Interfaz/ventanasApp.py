...
class IngresoPedido_gerente(creacion_ventana):
    # Define una clase que hereda de creacion_ventana y gestiona la creación de pedidos.
    def __init__(self, usuario):
        # Constructor de la clase.
        if not usuario:
            # Verifica que se proporcione un usuario válido.
        ...
        # Llama al constructor de la clase base y establece el título y el icono de la ventana.
        self.interfaz()  # Configura la interfaz gráfica.
        self.archivo_pedidos = "pedidostest.json"  # Ruta del archivo donde se guardan los pedidos.
        self.archivo_usuarios = "usuariostest.json"  # Ruta del archivo de usuarios.
        self.usuario = usuario  # Almacena el usuario actual.
    ...
    def interfaz(self):
        ...
        # Crea etiquetas y campos de entrada para capturar información del pedido.
        self.crear_etiqueta(texto="ID del usuario", fuente=("Arial"), tamano=12, x=120, y=120, bg="#188999", fg="#ffffff")
    ...
    def volver(self):
        # Cambia la ventana actual por la ventana del gerente.
    ...
    def guardar_pedido(self):
        ...
        fecha_creacion = fecha_actual.strftime("%Y-%m-%d %H:%M")  # Formatea la fecha de creación.
        fecha_entrega = (fecha_actual + timedelta(days=2)).strftime("%Y-%m-%d %H:%M")  # Calcula la fecha de entrega.

        # Obtiene los valores ingresados en los campos.
        usuario_codigo = self.entry_usuario.get().strip()
        ...
        # Validación de que la tarifa sea un número positivo.
        try:
        ...
        # Lee el archivo de usuarios para verificar la existencia del usuario.
        try:
        ...
        usuario_existente = next((u for u in usuarios if u.get("codigoUnico") == usuario_codigo), None)
        ...
        # Crea el diccionario del pedido con la información capturada.
        datos_pedido = {
        ...
        # Lee el archivo de pedidos y lo actualiza con el nuevo pedido.
        try:
        ...
        # Guarda los pedidos actualizados en el archivo.
        ...
        # Actualiza el archivo de usuarios con el nuevo pedido asociado.
    ...
    def generar_CodigoUnico(self):
        # Genera un código único aleatorio para un pedido.
        try:
            with open(self.archivo_pedidos, "r", encoding="utf-8") as archivo_pedidos:
                pedidos = json.load(archivo_pedidos)
    ...
    def volver(self):
        # Cambia a la ventana del gerente.
        self.cambiarVentana(ventana_gerente, self.usuario)
...
class ventana_EmpleadoRecaudador(creacion_ventana):
    ...
    def mostrar_detalles(self, codigo_entry, tarifa_var, usuario_var, confirmar_pago_button):
        ...
        if pedido and pedido['estadoActual'] == 'Listo para el retiro':
            ...
            confirmar_pago_button.config(state=tk.NORMAL)
        else:
    ...
    def pagar_pedidos(self):
        ...
        tk.Label(pago, text="Código Único").pack(pady=10)
        codigo_entry = tk.Entry(pago)
        codigo_entry.pack(pady=5)

        tarifa_var = tk.StringVar(value="Tarifa: N/A")
        usuario_var = tk.StringVar(value="Usuario: N/A")
        
        tk.Label(pago, textvariable=tarifa_var).pack(pady=5)
        tk.Label(pago, textvariable=usuario_var).pack(pady=5)

        confirmar_pago_button = tk.Button(pago, text="Confirmar Pago", state=tk.DISABLED, command=lambda: self.procesar_pago(codigo_entry, tarifa_var, usuario_var, confirmar_pago_button))
        confirmar_pago_button.pack(pady=20)

        tk.Button(pago, text="Mostrar Detalles", command=lambda: self.mostrar_detalles(codigo_entry, tarifa_var, usuario_var, confirmar_pago_button)).pack(pady=5)
...
class pedidosVer(creacion_ventana):
    ...
    def interfaz(self):
        ...
        # Función para copiar el código único al portapapeles
        def copiar_codigo(event):
            item = tabla.selection()[0]
            codigo_unico = tabla.item(item, 'values')[0]
            self.ventana.clipboard_clear()
            self.ventana.clipboard_append(codigo_unico)
