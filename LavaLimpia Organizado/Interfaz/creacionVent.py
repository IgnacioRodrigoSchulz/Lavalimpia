...
class creacion_ventana: #Esta clase es el origen de todas las otras ventanas de la aplicación, donde se establecen configuraciones en común
    ...
    def cambiarVentana(self, nuevaVentana):
        ...
        # Intentar instanciar la nueva ventana con o sin `usuario_actual`
        try:
            nueva_ventana = nuevaVentana(self.usuario_actual)  # Intenta pasar el usuario actual
        except TypeError:
            nueva_ventana = nuevaVentana()  # Si no necesita argumentos, crea la ventana sin ellos
